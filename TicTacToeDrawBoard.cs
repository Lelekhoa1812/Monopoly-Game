...
{
    public class TicTacToeDrawBoard
    {
        private PlayerType[,] draw;
        private List<TicTacToePlayer> markers = new List<TicTacToePlayer>();
        ...
        public TicTacToeDrawBoard(int windowWidth, int windowHeight, int rows, int cols)
        {
            Rows = rows;
            Cols = cols;
            SquareSize = windowWidth / cols;
            draw = new PlayerType[Rows, Cols];
            InitializeBoard();
        ...
        public void PlaceMarker()
        {
            ...
            if (row >= 0 && row < Rows && col >= 0 && col < Cols && draw[row, col] == PlayerType.Empty && _vsPlayerMode)
            {
                // Player X moves at odd-numbered and O moves at even-numbered moves
                var type = markers.Count % 2 == 0 ? PlayerType.X : PlayerType.O;
                draw[row, col] = type;
                markers.Add(new TicTacToePlayer(type));

            }

            //Player vs Machine Mode
            if (row >= 0 && row < Rows && col >= 0 && col < Cols && draw[row, col] == PlayerType.Empty && _vsMachineMode)
            {
                ...
                if (_vsMachineMode && type == PlayerType.X)
                {
                    ...
                    bool canCompleteStreak2O = FindStreak2(draw, PlayerType.O, out streakRow2O, out streakCol2O);

                    int streakRowO, streakColO;
                    bool canCompleteStreak3O = FindStreak3(draw, PlayerType.O, out streakRowO, out streakColO);

                    int stopRowX, stopColX;
                    bool canStopStreak3X = StopStreak3(draw, PlayerType.X, out stopRowX, out stopColX);
                    ...
                    if (markers.Count == 1)
                    {
                        ...
                        if (centerRow >= 0 && centerRow < Rows && centerCol >= 0 && centerCol < Cols && draw[centerRow, centerCol] == PlayerType.Empty)
                        {
                            draw[centerRow, centerCol] = PlayerType.O;
                            markers.Add(new TicTacToePlayer(PlayerType.O));
                        }
                        else
                        {
                            MakeMachineMove();
                        ...
                        if (canCompleteStreak3O)
                        {
                            ...
                            if (streakRowO >= 0 && streakRowO < Rows && streakColO >= 0 && streakColO < Cols)
                            {
                                ...
                                if (_horizontalStreak3O)
                                {
                                    ...
                                    if (streakColO + 1 >= 0 && draw[streakRowO, streakColO + 1] == PlayerType.Empty)
                                    {
                                        targetCol = streakColO + 1;
                                    }
                                    else if (streakColO - 3 < Cols && draw[streakRowO, streakColO - 3] == PlayerType.Empty)
                                    {
                                ...
                                else if (_verticalStreak3O)
                                {
                                    ...
                                    if (streakRowO + 1 >= 0 && draw[streakRowO + 1, streakColO] == PlayerType.Empty)
                                    {
                                        targetRow = streakRowO + 1;
                                    }
                                    else if (streakRowO - 3 < Rows && draw[streakRowO - 3, streakColO] == PlayerType.Empty)
                                    {
                                ...
                                else if (_TLBRStreak3O)
                                {
                                    ...
                                    if (streakRowO + 1 >= 0 && streakColO + 1 >= 0 && draw[streakRowO + 1, streakColO + 1] == PlayerType.Empty)
                                    {
                                        targetRow = streakRowO + 1;
                                        targetCol = streakColO + 1;
                                    }
                                    else if (streakRowO - 3 < Rows && streakColO - 3 < Cols && draw[streakRowO - 3, streakColO - 3] == PlayerType.Empty)
                                    {
                                        targetRow = streakRowO - 3;
                                        targetCol = streakColO - 3;
                                    }
                                ...
                                else if (_TRBLStreak3O)
                                {
                                    ...
                                    if (streakRowO - 1 >= 0 && streakColO - 1 < Cols && draw[streakRowO - 1, streakColO - 1] == PlayerType.Empty)
                                    {
                                        targetRow = streakRowO - 1;
                                        targetCol = streakColO - 1;
                                    }
                                    else if (streakRowO + 3 < Rows && streakColO + 3 >= 0 && draw[streakRowO + 3, streakColO + 3] == PlayerType.Empty)
                                    {
                                        targetRow = streakRowO + 3;
                                        targetCol = streakColO + 3;
                                    }
                                ...
                                if (targetRow >= 0 && targetRow < Rows && targetCol >= 0 && targetCol < Cols && draw[targetRow, targetCol] == PlayerType.Empty)
                                {
                                    draw[targetRow, targetCol] = PlayerType.O;
                                    markers.Add(new TicTacToePlayer(PlayerType.O));
                        ...
                        else if (canCompleteStreak2O && !canCompleteStreak3O && !canStopStreak3X)
                        {
                            ...
                            if (streakRow2O >= 0 && streakRow2O < Rows && streakCol2O >= 0 && streakCol2O < Cols)
                            {
                                ...
                                if (_horizontalStreak2O)
                                {
                                    ...
                                    if (streakCol2O + 1 >= 0 && draw[streakRow2O, streakCol2O + 1] == PlayerType.Empty)
                                    {
                                        targetCol2 = streakCol2O + 1;
                                    }
                                    else if (streakCol2O - 2 < Cols && draw[streakRow2O, streakCol2O - 2] == PlayerType.Empty)
                                    {
                                ...
                                else if (_verticalStreak2O)
                                {
                                    ...
                                    if (streakRow2O + 1 >= 0 && draw[streakRow2O + 1, streakCol2O] == PlayerType.Empty)
                                    {
                                        targetRow2 = streakRow2O + 1;
                                    }
                                    else if (streakRow2O - 2 < Rows && draw[streakRow2O - 2, streakCol2O] == PlayerType.Empty)
                                    {
                                ...
                                else if (_TLBRStreak2O)
                                {
                                    ...
                                    if (streakRow2O + 1 >= 0 && streakCol2O + 1 >= 0 && draw[streakRow2O + 1, streakCol2O + 1] == PlayerType.Empty)
                                    {
                                        targetRow2 = streakRow2O + 1;
                                        targetCol2 = streakCol2O + 1;
                                    }
                                    else if (streakRow2O - 2 < Rows && streakCol2O - 2 < Cols && draw[streakRow2O - 2, streakCol2O - 2] == PlayerType.Empty)
                                    {
                                        targetRow2 = streakRow2O - 2;
                                        targetCol2 = streakCol2O - 2;
                                    }
                                ...
                                else if (_TRBLStreak2O)
                                {
                                    ...
                                    if (streakRow2O - 1 >= 0 && streakCol2O - 1 < Cols && draw[streakRow2O - 1, streakCol2O - 1] == PlayerType.Empty)
                                    {
                                        targetRow2 = streakRow2O - 1;
                                        targetCol2 = streakCol2O - 1;
                                    }
                                    else if (streakRow2O + 2 < Rows && streakCol2O + 2 >= 0 && draw[streakRow2O + 2, streakCol2O + 2] == PlayerType.Empty)
                                    {
                                        targetRow2 = streakRow2O + 2;
                                        targetCol2 = streakCol2O + 2;
                                    }
                                ...
                                if (targetRow2 >= 0 && targetRow2 < Rows && targetCol2 >= 0 && targetCol2 < Cols && draw[targetRow2, targetCol2] == PlayerType.Empty)
                                {
                                    canCompleteStreak2O = false;
                                    draw[targetRow2, targetCol2] = PlayerType.O;
                                    markers.Add(new TicTacToePlayer(PlayerType.O));
                        ...
                        else if (canStopStreak3X && !canCompleteStreak3O)
                        {
                            ...
                            if (stopRowX >= 0 && stopRowX < Rows && stopColX >= 0 && stopColX < Cols)
                            {
                                ...
                                if (_horizontalStreakX)
                                {
                                    ...
                                    if (stopColX + 1 >= 0 && draw[stopRowX, stopColX + 1] == PlayerType.Empty)
                                    {
                                        atCol = stopColX + 1;
                                    }
                                    else if (stopColX - 3 < Cols && draw[stopRowX, stopColX - 3] == PlayerType.Empty)
                                    {
                                        atCol = stopColX - 3;
                                    }
                                }
                                ...
                                if (_verticalStreakX)
                                {
                                    ...
                                    if (stopRowX + 1 >= 0 && draw[stopRowX + 1, stopColX] == PlayerType.Empty)
                                    {
                                        atRow = stopRowX + 1;
                                    }
                                    else if (stopRowX - 3 < Rows && draw[stopRowX - 3, stopColX] == PlayerType.Empty)
                                    {
                                        atRow = stopRowX - 3;
                                    }
                                }
                                ...
                                if (_TLBRStreakX)
                                {
                                    ...
                                    if (stopRowX + 1 >= 0 && stopColX + 1 >= 0 && draw[stopRowX + 1, stopColX + 1] == PlayerType.Empty)
                                    {
                                        atRow = stopRowX + 1;
                                        atCol = stopColX + 1;
                                    }
                                    else if (stopRowX - 3 < Rows && stopColX - 3 < Cols && draw[stopRowX - 3, stopColX - 3] == PlayerType.Empty)
                                    {
                                        atRow = stopRowX - 3;
                                        atCol = stopColX - 3;
                                    }
                                ...
                                if (_TRBLStreakX)
                                {
                                    ...
                                    if (stopRowX - 1 >= 0 && stopColX - 1 < Cols && draw[stopRowX - 1, stopColX - 1] == PlayerType.Empty)
                                    {
                                        atRow = stopRowX - 1;
                                        atCol = stopColX - 1;
                                    }
                                    else if (stopRowX + 3 < Rows && stopColX + 3 >= 0 && draw[stopRowX + 3, stopColX + 3] == PlayerType.Empty)
                                    {
                                        atRow = stopRowX + 3;
                                        atCol = stopColX + 3;
                                    }
                                ...
                                if (atRow >= 0 && atRow < Rows && atCol >= 0 && atCol < Cols && draw[atRow, atCol] == PlayerType.Empty)
                                {
                                    draw[atRow, atCol] = PlayerType.O;
                                    markers.Add(new TicTacToePlayer(PlayerType.O));
            ...
            // 3 Player Mode
            else if (row >= 0 && row < Rows && col >= 0 && col < Cols && draw[row, col] == PlayerType.Empty && _threePlayerMode)
            {
        ...
        private void MakeMachineMove()
        {
            ...
            if (emptyCells.Count > 0)
            {
                ...
                draw[randomCell.Item1, randomCell.Item2] = PlayerType.O;
                markers.Add(new TicTacToePlayer(PlayerType.O));
                ...
                for (int c = 0; c < Cols; c++)
                {
                    if (draw[r, c] != PlayerType.Empty)
                    {
                        ...
                        if (r + 3 < Rows && c + 3 < Cols &&
                            draw[r, c] == draw[r + 1, c + 1] &&
                            draw[r, c] == draw[r + 2, c + 2] &&
                            draw[r, c] == draw[r + 3, c + 3])
                        {
                            winner = draw[r, c];
